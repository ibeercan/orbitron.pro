...
interface SolarArcFormProps {
  natalChartId: number
  onSubmit: (chart: Record<string, unknown>) => void
  onCancel: () => void
}

export function SolarArcForm({ natalChartId, onSubmit, onCancel }: SolarArcFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await chartsApi.createSolarArc(data)
      onSubmit(res.data)
    } catch (err: unknown) {
      const e = err as { response?: { data?: { detail?: string } } }
      setServerError(e.response?.data?.detail || 'Не удалось создать дирекции')
    } finally {
      ...
      <div className="flex gap-3 pt-1">
        ...
        <button type="submit" disabled={isSubmitting} className="btn-gold flex-1 h-11 text-sm flex items-center justify-center gap-2">
          {isSubmitting ? (
            ...
            <><Navigation className="w-4 h-4" />Построить</>
          )}
        </button>
      </div>
    </form>
