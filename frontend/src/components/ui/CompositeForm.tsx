...
import { Loader2, Merge, MapPin, UserPlus, Trash2 } from 'lucide-react'
...
export function CompositeForm({ natalChartId, onSubmit, onCancel, defaultType = 'composite' }: CompositeFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'saved' && selectedPersonId) {
        const res = await chartsApi.createComposite({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <FieldLabel>Тип карты</FieldLabel>
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setSynthesisType('composite')}
            className={cn(
          ...
          <button
            type="button"
            onClick={() => setSynthesisType('davison')}
            className={cn(
              'flex-1 h-9 rounded-xl text-xs font-medium transition-all',
              synthesisType === 'davison'
                ? 'bg-[rgba(212,175,55,0.1)] border border-[rgba(212,175,55,0.25)] text-[#D4AF37]'
                : 'text-[#8B7FA8] hover:text-[#D4AF37]'
            )}
          >
                ...
                {persons.map((p) => (
                  ...
                  >
                    <div className={cn(
                      ...
                      <Merge className={cn('w-3.5 h-3.5', selectedPersonId === p.id ? 'text-[#D4AF37]' : 'text-[#4A3F6A]')} />
                    </div>
      ...
      <div className="flex gap-3 pt-1">
        ...
        <button type="submit" disabled={isSubmitting || !canSubmit} className="btn-gold flex-1 h-11 text-sm flex items-center justify-center gap-2 disabled:opacity-50">
          {isSubmitting ? (
            ...
            <><Merge className="w-4 h-4" />Построить</>
          )}
        </button>
      </div>
    </form>
    ...
    <div className="space-y-4">
      ...
      <label className="flex items-center gap-2.5 cursor-pointer group">
        ...
        <span className="text-xs text-[#8B7FA8] group-hover:text-[#D4AF37] transition-colors flex items-center gap-1.5">
          ...
          Сохранить профиль для будущих карт
        </span>
      </label>
    </div>
  )
